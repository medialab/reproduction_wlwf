#install.packages(c("tidyverse", "tidyr", "ggplot2", "vars", "boot", "rio", "tseries", "argparse", "stringr", "data.table", "lubridate", "urca", "dplyr"))
...
if (args$estimate || args$tests || args$tests_post){
  ...
  db <- db[, c("topic", variables)]
  db <- na.omit(db, target.colnames = variables)
}
...
if (args$tests){
  ...
  titles <- read_csv("data_prod/figures/translate_number_name/pa2our_topics_crosswalk_merged_subissues.csv", col_names = FALSE, show_col_types=FALSE)
  titles <- titles[,1:2]
...
pa2our <- read_csv("data_prod/figures/translate_number_name/pa2our_topics_crosswalk_merged_subissues.csv", col_names=FALSE, show_col_types=FALSE)
pa2our <- pa2our[, 1:2]
...
agenda_type <- data.frame(
  var = variables,
  type = c("pol", "pol",  "pub", "pub", "pub", "media")
)
...
variables <- c('rep', 'dem', 'pubrep', 'pubdem', 'public', 'media')
readable_variables <- c("Republicains", 
                        "Democrats",
...
colors_dict <- c(
  "Républicains" = "red",
  "Democrats" = "blue",
  "Republican Supporters" = "coral",
  "Democrats Supporters" = "lightblue",
  "Public"= "darkorchid3", 
  "Media" = "green4"
)
