...
variables <- c('lr', 'majority', 'nupes', 'rn', 'lr_supp', 'majority_supp', 'nupes_supp', 'rn_supp','media')
...
if (args$tests){
  ...
  write.csv(statio_by_group, file="data_prod/var/issue-level/statio_details_noatt.csv",  row.names = FALSE)
  ...
  path_info <- "data_prod/var/issue-level/infos_topics_noatt.csv"
  write.csv(infos_topic, file=path_info, row.names = FALSE)
}

if (args$estimate){
  path_infos_topic <- "data_prod/var/issue-level/infos_topics_noatt.csv"
  ...
  for (topic_num in list_topic_iter){
    ...
    save(var_model, file = paste0("data_prod/var/issue-level/var_model_noatt_", topic_num, ".Rdata"))
    ...
    save(var_irfs_cum, file = paste0("data_prod/var/issue-level/var_girf_topic_noatt_", topic_num, ".Rdata"))
  }
}
...
if(args$tests_post){
  ...
  for (topic_num in list_topic_iter){
    ...
    var_path <- paste0("data_prod/var/issue-level/var_model_noatt_", topic_num, ".Rdata")
  ...
  path_post <-  "data_prod/var/issue-level/post_checks_noatt.csv"
  colnames(infos_topic_post) <- c("Topic", "Max_Modul", "Serial_AC", "Norm.")
...
last_top_path <- paste0("data_prod/var/issue-level/var_girf_topic_noatt_", last_topic, ".Rdata")
...
for (top in pol_issues) {
  ...
  file_name <- paste0("data_prod/var/issue-level/var_girf_topic_noatt_", top, ".Rdata")
...
agenda_type <- data.frame(
  var = variables,
  type = c("pol", "pol", "pol", "pol", "pub", "pub", "pub", "pub", "media")
)
...
png("data_prod/var/irf-analysis/number_leading_relations_pairs_noatt.png",width = 800, height = 600)
...
png("data_prod/var/irf-analysis/total_GIRF_relations_pairs_noatt.png",width = 800, height = 600)
...
png("data_prod/var/irf-analysis/figure4_noatt.png", width = 1600, height = 1400)       
p <- ggplot(plot_db,
...
png("data_prod/var/irf-analysis/girf_between_deputes_noatt.png",width = 1000, height = 800)
p <- ggplot(plot_db,
...
png("data_prod/var/irf-analysis/figure3_noatt.png",width = 1000, height = 1000)
p <- ggplot(plot_db,
...
png("data_prod/var/irf-analysis/figure6_noatt.png",width = 800, height = 800)
p <- ggplot(plot_db,
